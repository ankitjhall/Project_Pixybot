...
#define SYSCON_PDRUNCFG_PD_VDDFLASH      (1 << 11)		/*!< Flash Vdd */
#define SYSCON_PDRUNCFG_LP_VDDFLASH      (1 << 12)		/*!< Flash LP Vdd */
...
#define SYSCON_PDRUNCFG_PD_VDDHV_ENA     (1 << 18)		/*!< Vdd HV */
...
/**
 * Auto Clock Gating Overide definition bits
 */
#define SYSCON_AUTOCGOR_RAM0X           (1 << 1)		/*!< RAM0 and RAMX overide */
#define SYSCON_AUTOCGOR_RAM1            (1 << 2)		/*!< RAM1 overide */
#define SYSCON_AUTOCGOR_RAM2            (1 << 3)		/*!< RAM2 overide */
#define SYSCON_AUTOCGOR_MASK            (SYSCON_AUTOCGOR_RAM0X | SYSCON_AUTOCGOR_RAM1 | SYSCON_AUTOCGOR_RAM2)

/**
 * @brief	Disables Auto clock gating for SRAM's
 * @param	mask	: Mask of RAM bits
 * @return	Nothing
 */
__STATIC_INLINE void Chip_SYSCON_DisableAutoClocking(uint32_t mask)
{
	LPC_SYSCON->AUTOCGOR = (LPC_SYSCON->AUTOCGOR & SYSCON_AUTOCGOR_MASK) | mask;
}

/**
 * @brief	Re-enables Auto clock gating for SRAM's
 * @param	mask	: Mask of RAM bits
 * @return	Nothing
 */
__STATIC_INLINE void Chip_SYSCON_EnableAutoClocking(uint32_t mask)
{
	LPC_SYSCON->AUTOCGOR = (LPC_SYSCON->AUTOCGOR & SYSCON_AUTOCGOR_MASK & ~mask);
}
