/*
 * @brief LPC5411X RTC chip driver
 *
 * @note
...
#ifndef __RTC_5411X_H_
#define __RTC_5411X_H_

#ifdef __cplusplus
extern "C" {
#endif

/** @defgroup RTC_5411X CHIP: LPC5411X Real Time clock
 * @ingroup CHIP_5411X_DRIVERS
 * @{
 */

/**
 * @brief LPC5411X Real Time clock register block structure
 */
typedef struct {			/*!< RTC */
...
#define RTC_CTRL_RTC_OSC_PD     (1 << 8)	/*!< RTC power-down bit */
#define RTC_CTRL_RTC_OSC_BYPASS (1 << 9)	/*!< RTC power-down bit */
#define RTC_CTRL_MASK           ((uint32_t) 0x0000003FD)	/*!< RTC Control register Mask for reserved bits */
...
/**
 * @brief	Power up the RTC
 * @param	pRTC	: The base address of RTC block
 * @return	Nothing
 */
__STATIC_INLINE void Chip_RTC_PowerUp(LPC_RTC_T *pRTC)
{
	Chip_RTC_DisableOptions(pRTC, RTC_CTRL_RTC_OSC_PD);
}
...
{
	Chip_RTC_EnableOptions(pRTC, RTC_CTRL_RTC_OSC_PD);
}
 ...
 * @return	Nothing
 * @note	Use and OR'ed stsMask value of RTC_CTRL_ALARM1HZ,
