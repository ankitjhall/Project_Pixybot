...
ErrorCode_t ADC_iso_out_hdlr(USBD_HANDLE_T hUsb,
																		  void *data,
																		  uint32_t event)
{
	ADC_SUBSTREAM_T *pSubs = (ADC_SUBSTREAM_T *) data;
	...
	if ( /*(pAdcCtrl->flags & ADC_PLAYING) &&*/ (event == USB_EVT_OUT)) {
		...
		do {
			...
			*addr = (1ul << 31)		/* buffer active */
					| (1ul << 26)							/* ISOCH endpoint type */
					| ((pSubs->maxp << 16)
					   | (0xFFFF & (pSubs->eTD[idx].buf_ptr >> 6)));
		} while ((pSubs->epQH[pSubs->usb_idx] & (1UL << 31)) == 0);
...
ErrorCode_t ADC_init(ADC_INIT_PARAM_T *adc_param)
{
	...
	USB_ADC_CTRL_T *pAdcCtrl = &g_AdcCtrl;
	EP_Queue_T *epQueue = (EP_Queue_T *) LPC_USB->EPLISTSTART;
	...
	mem_required += (MAX_SUBSTREAMS * 0x3F);	/* padding memory used for alignment */
	...
	for (i = 0; i < MAX_SUBSTREAMS; i++) {
		...
		if (adc_param->ep_num[i] != 0) {
			...
			pSubs->ep_index =
				((adc_param->ep_num[i] & 0xF) << 1) + i;		/* endpoint control bit */
...
/** ADC_start_xfr: */
//__attribute__ ((section(".critical_text")))
void ADC_start_xfr(USB_ADC_CTRL_T *pAdcCtrl,
															   ADC_SUBSTREAM_T *pSubs,
															   uint32_t mode)
{
	...
	LPC_USB->EPSKIP = ep_bit;
	while (LPC_USB->EPSKIP & ep_bit) {	/* wait until skip is cleared */

	}
	pSubs->epQH[0] &= ~(1ul << 31);		/* clear active bit */
	pSubs->epQH[1] &= ~(1ul << 31);		/* clear active bit */
	LPC_USB->EPINUSE &= ~ep_bit;		/* reset the in buffer index to 0*/
