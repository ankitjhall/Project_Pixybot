...
/**
 * @brief   Gets the clock divider used by USB
 * @return	Returns clock divider used by USB
 */
__STATIC_INLINE uint32_t Chip_Clock_GetUSBClockDiv(void)
{
	return (LPC_SYSCON->USBCLKDIV & 0xFF)+1;
}
...
__STATIC_INLINE void Chip_Clock_SetMCLKClockSource(CHIP_SYSCON_MCLKSRC_T src, uint32_t div)
{
	...
	LPC_SYSCON->MCLKDIV     = (uint32_t) ((div-1) & 0xff);
}
...
{
	return (LPC_SYSCON->MCLKDIV & 0xff) + 1;
}
 ...
 * @return	Nothing
 * @note	The CLKOUT clock rate is the clock source divided by the divider.
 */
...
/**
 * @brief	Get CLKOUT clock source
 * @return	Clock output source
 */
__STATIC_INLINE CHIP_SYSCON_CLKOUTSRC_T Chip_Clock_GetCLKOUTSource(void)
{
	return (CHIP_SYSCON_CLKOUTSRC_T)(LPC_SYSCON->CLKOUTSELA & 0x07);
}

/**
 * @brief	Get CLKOUT clock divider
 * @return	Clock output source
 */
__STATIC_INLINE uint32_t Chip_Clock_GetCLKOUTDiv(void)
{
	return ((LPC_SYSCON->CLKOUTDIV&0xff)+1);
}
...
{
	return (LPC_SYSCON->SYSTICKCLKDIV & 0xff);
}
 ...
 * @param	div	: divider for system clock
 * @return	Nothing
 * @note	Use a divider value of 1 to 255. The system clock
 ...
 * @param	None
 * @return	System clock divider
 */
__STATIC_INLINE uint32_t Chip_Clock_GetSysClockDiv(void)
{
	return ((LPC_SYSCON->AHBCLKDIV & 0xff)+1);
}
...
{
	return (LPC_SYSCON->RTCOSCCTRL) ? true : false;
}
